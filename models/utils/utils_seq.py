"""
Sequence model utility functions.
...
def load_text_processor(fname='title_pp.dpkl'):
    ...
    ----------
    fname: str
        filename proccessor object

    Returns
    -------
    num_tokens : int
        size of vocabulary loaded into processor
    pp : ktext.processor
        processor to be loaded
...
def load_decoder_inputs(decoder_np_vecs='train_title_vecs.npy'):
    ...
    ----------
    decoder_np_vecs : str
        Filename of serialized numpy.array of decoder input (question title).
    ...
    # when training using Teacher Forcing.
...
def load_encoder_inputs(encoder_np_vecs='train_body_vecs.npy'):
    ...
    ----------
    encoder_np_vecs : str
        filename of serialized numpy.array of encoder input (question title)
...
class Inference(object):
    ...
    def __init__(self,
        ...
        self.encoder_model = extract_encoder_model(trained_model)
        self.decoder_model = extract_decoder_model(trained_model)
        self.default_title_maxlen = self.pp_title.padding_maxlen
        self.nn = None
        self.rec_df = None
    ...
    def generate_title(self,
        ...
        ----------
        raw_input: str
            The body of the question text as an input string
    ...
    def print_example(self,
        ...
        emb, gen_title = self.generate_title(body_text)
        ...
        if self.nn:
            ...
            if min(dist) <= threshold:
                cols = ['link', 'title', 'body']
                dfcopy = self.rec_df.iloc[neighbors][cols].copy(deep=True)
                dfcopy['dist'] = dist
                similar_questions_df = dfcopy.query('dist <= {}'.format(threshold))

                print("\n**** Similar Questions (using encoder embedding) ****:\n")
                display(similar_questions_df)
    ...
    def demo_model_predictions(self,
        ...
        ----------
        n: int
            Number of questions to display from question_df
        question_df : pandas DataFrame
            DataFrame that contains two columns: `body` and `title`.
        threshold : float
            distance threshold for recommendation of similar questions.
        ...
        title_text = question_df.title.tolist()
        url = question_df.link.tolist()
    ...
    def prepare_recommender(self, vectorized_array, original_df):
        ...
        vectorized_array : List[List[int]]
            ...
            that is fed into the model for training.
        ...
        emb = self.encoder_model.predict(
            x=vectorized_array, batch_size=vectorized_array.shape[0]//200)
    ...
    def evaluate_model(self, holdout_bodies, holdout_titles):
        ...
        for i in tqdm_notebook(range(num_examples)):
            _, yhat = self.generate_title(holdout_bodies[i])
        ...
        # expects List[List[List[str]]] for ground truth, using List[List[str]]
        # will give you erroneous results.
