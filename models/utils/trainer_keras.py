...
from utils.utils_seq import load_decoder_inputs
from utils.utils_seq import load_encoder_inputs
from utils.utils_seq import load_text_processor
...
class Trainer(object):
    ...
    def preprocess(self, data_glob, num_samples=None):
        ...
        if num_samples:
            traindf, self.test_df = train_test_split(
                pd.concat([pd.read_csv(f, usecols=['body', 'title', 'link'])
                                for f in data_glob],
    ...
    def evaluate(self):
        ...
        bleu_score = inference.evaluate_model(
            ...
            holdout_titles=self.test_df.title.tolist())
